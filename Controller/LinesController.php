...
class LinesController extends AppController {
    ...
    function admin_add() {
        if (!empty($this->data)) {
            $this->Line->create();
            $this->request->data['Line']['user_id'] = $this->Auth->user('id');
            $this->request->data['Line']['author'] = $this->Auth->user('username');
    ...
    function admin_edit($id = null) {
        if($id == $this->Auth->user('id') || $this->Auth->user('id') == 1) {
            ...
            $this->Session->setFlash(__('You\'re not the owner of this line. Permission Denied.', true));
            $this->redirect(array('action' => 'index'));
        }
    }
